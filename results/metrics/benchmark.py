"""
benchmark.py  —  Member 8: Comparison & Quantitative Analysis
SBEG108 · Numerical Methods in Biomedical Engineering
Cairo University · Spring 2026

RK4 is the reference solution.
Bar charts show only RK45 and PINN errors relative to RK4.

Input files (place in same folder as this script):
    rk4_results.csv          columns: t, G, I, wall_time_sec
    RK45_Results.csv         columns: t, G, I, wall_time
    all_predictions.csv      columns: t_hours, G_RK45, G_PINN, G_error,
                                      I_RK45, I_PINN, I_error, region
    metrics.csv              columns: metric, value, units
                                      rows: RMSE_G, RMSE_I, MAE_G, MAE_I, best_loss
    inference_time.csv       columns: run, inference_time_s, inference_time_ms

Optional bonus figures:
    dense_predictions.csv    columns: same as all_predictions.csv  (region = training)
    extrapolation.csv        columns: same as all_predictions.csv  (region = extrapolation)

Outputs (same folder):
    comparison_table.csv
    benchmark_metrics.csv
    figures/rmse_bar_chart.png
    figures/mae_bar_chart.png
    figures/timing_bar_chart.png
    figures/trajectories_comparison.png
    figures/pinn_error_profile.png       (requires dense_predictions.csv)
    figures/pinn_extrapolation.png       (requires dense_predictions.csv + extrapolation.csv)
"""
...
# ── PINN accuracy metrics (pre-computed by Member 7) ─────────────────────
# metrics.csv rows: RMSE_G, RMSE_I, MAE_G, MAE_I (computed on training window vs RK45)
pinn_rmse_G = pinn_rmse_I = pinn_mae_G = pinn_mae_I = float("nan")
if os.path.exists(METRICS_CSV):
    met = pd.read_csv(METRICS_CSV).set_index("metric")["value"]
    pinn_rmse_G = float(met.get("RMSE_G", float("nan")))
    pinn_rmse_I = float(met.get("RMSE_I", float("nan")))
    pinn_mae_G  = float(met.get("MAE_G",  float("nan")))
    pinn_mae_I  = float(met.get("MAE_I",  float("nan")))
    print(f"  {'metrics.csv':<28s}: RMSE_G={pinn_rmse_G:.6f}  RMSE_I={pinn_rmse_I:.6f}")
else:
    print("  [WARNING] metrics.csv not found — computing PINN metrics from all_predictions.csv")
    # Fall back: compute from the pre-computed error columns in training region
    if len(train_df) > 0 and "G_error" in train_df.columns:
        pinn_rmse_G = float(np.sqrt((train_df["G_error"] ** 2).mean()))
        pinn_rmse_I = float(np.sqrt((train_df["I_error"] ** 2).mean()))
        pinn_mae_G  = float(train_df["G_error"].abs().mean())
        pinn_mae_I  = float(train_df["I_error"].abs().mean())

# ── Interpolate RK45 onto RK4 time grid for fair comparison ──────────────
print("\nInterpolating RK45 onto RK4 time grid (reference) …")
...
# G_error and I_error columns are |PINN − RK45| pre-computed by Member 7.
